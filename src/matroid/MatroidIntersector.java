...
public class MatroidIntersector<Type, Subset1, Subset2> {
	
	// Computes a maximal set in both matroids
	// It is assumed they have the same ground set
	public ArrayList<Type> intersection(Matroid<Type, Subset1> m, Matroid<Type, Subset2> n){
		...
		Subset1 maximalSet_Subset1;
		Subset2 maximalSet_Subset2;
		OrientedGraph G;	
		
		// Improve 'maximalSet'
		maximalSetWhile:
		while (maximalSetSize < groundSetSize) {
			// Used for the matroid methods
			maximalSet_Subset1 = m.arrayToSubset(maximalSet);
			maximalSet_Subset2 = n.arrayToSubset(maximalSet);
			...
			for (int i = maximalSetSize; i < groundSetSize; i++) {
				if (m.belongsTo(maximalSet_Subset1, maximalSetC.get(i - maximalSetSize))) {

					// Check if the set maximalSet U {maximalSetC.get(i - maximalSetSize)}
					// belongs to both matroids
					if (n.belongsTo(maximalSet_Subset2, maximalSetC.get(i - maximalSetSize))) {
				...
				else if (n.belongsTo(maximalSet_Subset2, maximalSetC.get(i - maximalSetSize))) {
					history[i] = -2;
				}
				else {
					history[i] = -3;
			...
			G = new OrientedGraph(groundSetSize);				
			
			for (int i = 0; i < maximalSetSize; i++) {
				for (int j = maximalSetSize; j < groundSetSize; j++) {
					if (m.belongsTo(maximalSet_Subset1, maximalSetC.get(j - maximalSetSize), maximalSet.get(i))) G.addEdge(i, j);
					if (n.belongsTo(maximalSet_Subset2, maximalSetC.get(j - maximalSetSize), maximalSet.get(i))) G.addEdge(j, i);
				}
			}
